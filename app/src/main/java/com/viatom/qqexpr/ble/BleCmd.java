...
public class BleCmd {
	...
	public static byte[] getCmd(int model, int msgType) {
		if(model == Bluetooth.MODEL_PC100) {
			return PcCmd.getCmd(msgType);
		} else {
			return new byte[0];
		}
	}

	public static int getMsgType(int model, byte[] response) {
		if(model == Bluetooth.MODEL_PC100) {
			return PcCmd.getMsgType(response);
		} else {
	...
	public static class PcCmd {
		...
		public static byte[] getCmd(int msgType) {
			switch(msgType) {
				case PcCmd.CMD_TYPE_HAND_SHAKE:
					return handshake();
				case PcCmd.CMD_TYPE_GET_DEVICE_INFO:
					return getDeviceInfo();
				case PcCmd.CMD_TYPE_GET_DEVICE_ID:
					return getDeviceId();
				case PcCmd.CMD_TYPE_SET_IDLE:
					return setIdle();
				case PcCmd.CMD_TYPE_SHUT_DOWN:
					return shutdown();
				case PcCmd.CMD_TYPE_BP_START:
					return startBp();
				case PcCmd.CMD_TYPE_BP_END:
					return stopBp();
				case PcCmd.CMD_TYPE_BP_GET_RESULT:
					return getBpResult();
				case PcCmd.CMD_TYPE_BP_GET_STATE:
					return getBpState();
				case PcCmd.CMD_TYPE_SPO2_START:
					return startSpo2();
				case PcCmd.CMD_TYPE_SPO2_END:
					return stopSpo2();
				case PcCmd.CMD_TYPE_SPO2_GET_STATE:
					return getSpo2State();
				default:
					return new byte[0];
			}
		...
		public static int getMsgType(byte[] response) {
			Response.PcBleResponse bleResponse = new Response.PcBleResponse(response);
		...
		private static byte[] handshake() {
			...
			cmd[4] = (byte) 0x01;
			// length
			cmd[5] = CrcUtil.calCRC8ByCCITT(Arrays.copyOfRange(cmd, 0, cmd.length - 1));
			return cmd;
		}
...
//			cmd[4] = (byte) 0x01;
//
//			cmd[5] = CrcUtil.calCRC8(Arrays.copyOfRange(cmd, 0, cmd.length - 1), Table_CRC8_CCITT);
